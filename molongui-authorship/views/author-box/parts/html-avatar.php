<?php

use Molongui\Authorship\Common\Utils\Plugin;

defined( 'ABSPATH' ) or exit; // Exit if accessed directly
?>

<?php if ( $profile['avatar'] and !empty( $options['author_box_avatar_show'] ) ) : ?>
	<div class="m-a-box-item m-a-box-avatar" data-source="<?php echo $options['author_box_avatar_source']; ?>">
		<?php
            if ( empty( $options['author_box_avatar_link'] ) or 'none' === $options['author_box_avatar_link']
                 or
                 ( 'website' === $options['author_box_avatar_link'] and empty( $profile['web'] ) )
                 or
                 ( 'custom' === $options['author_box_avatar_link'] and empty( $profile['custom_link'] ) )
                 or
                 ( 'archive' === $options['author_box_avatar_link']
                    and
                    ( ( 'guest' === $profile['type'] and !Plugin::has_pro() )
                        or
                      ( 'guest' === $profile['type'] and !$options['guest_pages'] )
                        or
                      ( 'user' === $profile['type'] and !$options['user_archive_enabled'] )
                    )
                 )
            ){
	            ?>
                <span>
                    <?php echo $profile['avatar']; ?>
                </span>
                <?php
            }
            else
            {
                switch ( $options['author_box_avatar_link'] )
                {
                    case 'website': $url = $profile['web']; break;
                    case 'custom' : $url = $profile['custom_link']; break;
                    case 'archive': default: $url = $profile['archive_url']; break;
                }
                ?>
                <a class="m-a-box-avatar-url" href="<?php echo esc_url( $url ); ?>">
                    <?php echo $profile['avatar']; ?>
                </a>
                <?php
            }
        ?>
	</div>
<?php endif; ?>
